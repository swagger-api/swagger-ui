...
<a name="withCredentials"></a>`withCredentials` | `WITH_CREDENTIALS` | `Boolean=false` if set to true, enables passing credentials in CORS requests. e.g. Cookies and Authorization headers that are set by the browser. Note that swagger-ui can set headers cross-domain but cannot currently set cookies cross-domain. That means you can authenticate with headers cross-domain without this, but you cannot authenticate with cookies cross-domain and will have to rely on the browser supplied cookies, that this setting enables sending, and that swagger-ui cannot control.
