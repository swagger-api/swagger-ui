...
<a name="saveAuthorization"></a>`saveAuthorization` | `SAVE_AUTHORIZATION` | `Boolean=false`. If set to `true`, enables to save the value entered in authorize on localStorage of the browser. The `key` attribute is written with the prefix `swagger-` complemented for variable "Name" currently used.
