...
SwaggerApi.prototype.build = function() {
  var _this = this;
  this.progress('Carregando documentação: ' + this.url);
  var obj = {
  ...
  for (resource_name in _ref) {
    ...
    for (operation_name in _ref1) {
      ...
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        parameter = _ref2[_i];
        log("    " + parameter.name + (parameter.required ? ' (obrigatório)' : '') + " - " + parameter.description);
      }
    }
  }
  return this;
...
var SwaggerResource = function(resourceObj, api) {
  ...
  if ((resourceObj.apis != null) && (this.api.resourcePath != null)) {
    ...
    this.api.progress('Carregando documentação ' + this.name + ': ' + this.url);
    obj = {
      url: this.url,
      method: "get",
      useJQuery: this.useJQuery,
...
SwaggerResource.prototype.help = function() {
  ...
  for (operation_name in op) {
    ...
    for (var i = 0; i < operation.parameters; i++) {
      ...
      msg.concat("    " + parameter.name + (parameter.required ? ' (obrigatório)' : '') + " - " + parameter.description);
    }
    output.push(msg);
  }
  return output;
...
var SwaggerModelProperty = function(name, obj) {
  ...
  if (obj.allowableValues != null) {
    ...
    if (this.values != null) {
      this.valuesString = "'" + this.values.join("' ou '") + "'";
    }
  }
  if (obj["enum"] != null) {
    ...
    if (this.values != null) {
      this.valueString = "'" + this.values.join("' ou '") + "'";
    }
  }
}
...
SwaggerModelProperty.prototype.toString = function() {
  ...
  if (!this.required) {
    str += ', <span class="propOptKey">opcional</span>';
  }
  str += ')';
  if (this.values != null) {
    str += " = <span class='propVals'>['" + this.values.join("' ou '") + "']</span>";
  }
  if (this.descr != null) {
...
SwaggerOperation.prototype.help = function() {
  ...
  for (var i = 0; i < params.length; i++) {
    ...
    if (msg !== "")
      msg += "\n";
    msg += "* " + param.name + (param.required ? ' (obrigatório)' : '') + " - " + param.description;
  }
  return msg;
};
