 ...
 */
(function(){
var ArrayModel = function(definition, allDefinitions) {
  ...
  if(innerType) {
    ...
    this.properties.push(new Property('', innerType, true));
  }
  return this;
};
...
ArrayModel.prototype.getMockSignature = function(modelsToIgnore) {
  ...
  for (i = 0; i < this.properties.length; i++) {
    prop = this.properties[i];
    var ref = prop.$ref;
    if (!ref && prop.mapValue) {
      ref = prop.mapValue.$ref;  
    }
...
SwaggerClient.prototype.buildFromSpec = function(response) {
  ...
  for(key in this.definitions) {
    var model = new Model(key, this.definitions[key], this.definitions);
    if(model) {
      models[key] = model;
    }
  }
...
var Operation = function(parent, scheme, operationId, httpMethod, path, args, definitions) {
  ...
  if(definitions) {
    ...
    var key;
    for(key in this.definitions) {
      model = new Model(key, definitions[key], this.definitions);
      if(model) {
        models[key] = model;
      }
    }
...
Operation.prototype.resolveModel = function (schema, definitions) {
  if(typeof schema.$ref !== 'undefined') {
    ...
    if(definitions[ref]) {
      return new Model(ref, definitions[ref], definitions);
    }
  }
  if(schema.type === 'array')
    return new ArrayModel(schema, definitions);
  else
    return null;
};
...
var Model = function(name, definition, allDefinitions) {
  this.name = name;
  this.definition = definition || {};
  if(definition.allOf) {
    var resolve = function(definition, updatedDefinition, ignoreModels) {
      if (!definition)
        return;
      
      if (definition.allOf) {
        for (var i=0,len=definition.allOf.length;i<len;i++) {
          resolve(definition.allOf[i], updatedDefinition, ignoreModels);
        }
      } else if (definition.$ref) {
        var simpleName = simpleRef(definition.$ref);
        if (ignoreModels[simpleName] !== true) {
          var newIgnore = {};
          newIgnore[simpleName] = true;
          $.extend(newIgnore, ignoreModels);
          resolve(allDefinitions[simpleName], updatedDefinition, newIgnore);
        }
      } else {
        updatedDefinition.required = updatedDefinition.required || [];
        updatedDefinition.properties = updatedDefinition.properties || {};
        $.extend(updatedDefinition.required, definition.required);
        $.extend(updatedDefinition.properties, definition.properties);
      }
    };
    resolve(definition, this.definition, {});
  }

  this.properties = [];
  var requiredFields = definition.required || [];
  if(definition.type === 'array') {
    var out = new ArrayModel(definition, allDefinitions);
    return out;
  }
  var key;
...
var Property = function(name, obj, required) {
  ...
  if(obj.additionalProperties) {
    this.mapValue = new Property('map', obj.additionalProperties, required);
  }
  this.optional = true;
...
Property.prototype.sampleValue = function(isArray, ignoredModels, representations) {
  ...
  else if(this.mapValue) {
    output = {
      "key": this.mapValue.sampleValue(false, {}, representations)
    };
  }
...
getStringSignature = function(obj, baseComponent) {
  ...
  if(typeof obj.$ref !== 'undefined')
    ...
    var prop = new Property('map', obj.additionalProperties, false);
    str += 'Map[';
    str += getStringSignature(obj.additionalProperties);
    str += ']';
  }
  else
    str += obj.type;
