  ...
  if(typeof schema['$ref'] !== 'undefined') {
    ...
    if(definition) {
      if(definition.type === 'array') {
        return new ArrayModel(definition);
      }
      return new Model(ref, definition);
    }
  }
...
Operation.prototype.getSignature = function(type, models) {
  ...
  // handle shorthand [Type] array format
  ...
  if (model) {
    // handle json-schema array format
    if (model.definition.type === 'array') {
      listType = model.definition.items.$ref;
      return models[listType].getMockSignature();
    }
...
Operation.prototype.getSampleJSON = function(type, models) {
  ...
  if (type instanceof Array) {
    listType = type[0];
    return JSON.stringify([models[listType].createJSONSample()], null, 2);
  }
  ...
  if (model) {
    ...
    // handle xml
    if(model.nodeName) {
      var xmlString = new XMLSerializer().serializeToString(model);
